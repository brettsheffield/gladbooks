...
<xsl:output method="text" disable-output-escaping="yes" />
	...
	<xsl:include href="../purchaseinvoice/purchaseinvoice.xsl"/>

	<xsl:template match="purchaseorder">
		<xsl:call-template name="setSearchPath"/>

                <xsl:if test="not(@id)">
                        <xsl:text>INSERT INTO purchaseorder (organisation, authuser, clientip) VALUES (</xsl:text>
			<xsl:choose>
		...
		<xsl:text>INSERT INTO purchaseorderdetail (purchaseorder,</xsl:text>
		...
		<xsl:text>authuser,clientip) VALUES (</xsl:text>
                ...
                <xsl:choose>
                        ...
                        <xsl:otherwise>
                                <xsl:text>currval(pg_get_serial_sequence('purchaseorder','id')),'</xsl:text>
                        </xsl:otherwise>
                </xsl:choose>
		...
		<xsl:apply-templates select="salesitem">
			<xsl:with-param name="parentobject" select="'purchaseorder'" />
		</xsl:apply-templates>

		<!-- add any purchaseinvoices -->
